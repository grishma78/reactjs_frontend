...
export default class MainLayout extends Component {
  componentDidMount() {
    ...
    axios.get("http://localhost:8082/listEmp").then(res => {
      console.log(res);
  ...
  renderTable = () => {
    ...
    return this.state.posts.map(value => {
  ...
  showform() {
    //display form in place of table
    ReactDOM.render(<employeeAdd />, document.getElementById("tabid"));
  }
  showChart() {
    ReactDOM.render(<DisplayChart />, document.getElementById("tabid"));
  }

  render() {
    //table heading and call function to display contents
    return (
      <Router>
        <div className="split1 right">
          <div className="centered">
            <div id="tabid">
              ...
              {this.state && this.state.posts && this.renderTable()}
              <Link to={"/createEmp"}>
            ...
            <Switch>
              ...
              <Route exact path="/chart" component={DisplayChart} />
            </Switch>
          </div>
        </div>
      </Router>
